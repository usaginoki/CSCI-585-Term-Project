...
import source.audio_face_transformer.preprocess_main as tr_data
import source.audio_face_transformer.utils as tr_utils
import source.audio_face_transformer.tr_config as tr_config
...
TUNE_TARGET = tr_config.TUNE_TARGET
TUNE_MODE = tr_config.TUNE_MODE
TUNER_CSV_SAVE_PATH = config.AUDIO_FACE_TRANSFORMER_TUNER_CSV_SAVE_PATH
TUNER_SAVE_PATH = config.AUDIO_FACE_TRANSFORMER_TUNER_SAVE_PATH
BEST_HP_JSON_SAVE_PATH = config.AUDIO_FACE_TRANSFORMER_BEST_HP_JSON_SAVE_PATH
TUNE_HP_RANGES = tr_config.tune_hp_ranges
MAX_TRIALS = tr_config.max_trails

INITIAL_LR = tr_config.lr
INITIAL_EPOCH = tr_config.initial_epoch
REDUCE_LR_FACTOR = tr_config.reduce_lr_factor
REDUCE_LR_PATIENCE = tr_config.reduce_lr_patience
EARLY_STOPPING_PATIENCE = tr_config.early_stopping_patience_epoch
SOFTMAX_LEN = tr_config.softmax_len
...
MODEL_SAVE_PATH = config.AUDIO_FACE_TRANSFORMER_MODEL_SAVE_PATH
...
MAX_THREADS = tr_config.MAX_THREADS
...
class CustomModelBase(pt_train.CustomModelBase):
    ...
    def training_step(self, batch):
        (
            all_frames_face_images_X,
            all_frames_face_lands_X,
            all_extracted_mfcc_list,
            all_text_sentiment_X,
            all_video_emotions_Y,
        ) = batch
        out = self(
            all_frames_face_images_X,
            all_frames_face_lands_X,
        ...
        loss = F.cross_entropy(
            out, all_video_emotions_Y, weight=self.class_weights
...
class CustomModel(CustomModelBase):
    """
    Custom model for audio and face combined model using a Transformer encoder-decoder architecture.
    """
    ...
    def __init__(
        ...
        # MLP layers to project inputs to d_model dimension
        audio_layers = []
        for i in range(num_layers):
            if i == 0:
                audio_layers.append(nn.Linear(audio_last_layer_num, dense_units))
            elif i == num_layers - 1:
                audio_layers.append(nn.Linear(dense_units, d_model))
            else:
                audio_layers.append(nn.Linear(dense_units, dense_units))
            audio_layers.append(nn.ReLU())
            audio_layers.append(nn.Dropout(dropout_rate))
            if layers_batch_norm:
                audio_layers.append(
                    nn.BatchNorm1d(dense_units if i < num_layers - 1 else d_model)
                )
        self.project_audio = nn.Sequential(*audio_layers)

        image_layers = []
        for i in range(num_layers):
            if i == 0:
                image_layers.append(nn.Linear(face_last_layer_num, dense_units))
            elif i == num_layers - 1:
                image_layers.append(nn.Linear(dense_units, d_model))
            else:
                image_layers.append(nn.Linear(dense_units, dense_units))
        ...
        self.project_image = nn.Sequential(*image_layers)

        text_layers = []
        for i in range(num_layers):
            if i == 0:
                text_layers.append(nn.Linear(input_shapes[2], dense_units))
            elif i == num_layers - 1:
                text_layers.append(nn.Linear(dense_units, d_model))
            else:
                text_layers.append(nn.Linear(dense_units, dense_units))
        ...
        self.project_text = nn.Sequential(*text_layers)
        ...
        self.fc = nn.Sequential(
            ...
            nn.BatchNorm1d(dense_units) if layers_batch_norm else nn.Identity(),
        )
    ...
    def forward(self, input_image_X1, input_image_X2, input_audio_X, input_text_X):
        ...
        output_audio = self.project_audio(output_audio).unsqueeze(
            0
        ...
        for i in range(self.num_images):
            ...
            output_image = self.project_image(output_image)
            image_outputs.append(output_image)
        image_sequence = torch.stack(image_outputs)  # (num_images, batch_size, d_model)

        # Process text input
        output_text = self.project_text(input_text_X).unsqueeze(
            0
        )  # (1, batch_size, d_model)

        # Combine inputs for encoder
        encoder_input = torch.cat(
            [output_audio, image_sequence, output_text], dim=0
        )  # (num_images + 2, batch_size, d_model)

        # Use the first token (audio) as the target for the decoder
        decoder_input = output_audio

        # Pass through Transformer
        output = self.transformer(encoder_input, decoder_input)

        # Use the last token of the output for classification
        output = output[-1]
...
class FaceModelNoSoftmax(face_model.CustomModel):
    ...
    def __init__(self, face_model_save_path, **kwargs):
        ...
        if os.path.exists(face_model_save_path):
            ...
            input(
                "Face model not found. Press enter to continue training with random weights."
            )
...
class AudioModelNoSoftmax(audio_model.CustomModel):
    ...
    def __init__(self, audio_model_save_path, **kwargs):
        ...
        if os.path.exists(audio_model_save_path):
            ...
            input(
                "Audio model not found. Press enter to continue training with random weights."
            )
...
def get_face_model(
    best_hp_json_save_path=FACE_BEST_HP_JSON_SAVE_PATH,
    ...
    input_shape_1 = tr_utils.get_input_shape(which_input="image")
    input_shape_2 = tr_utils.get_input_shape(which_input="landmarks_depths")
    ...
    with torch.no_grad():
        ...
        input_image_X1 = torch.rand(
            batch_size, input_shape_1[0], input_shape_1[1], input_shape_1[2]
        ).to(device)
...
def get_audio_model(
    best_hp_json_save_path=AUDIO_BEST_HP_JSON_SAVE_PATH,
    ...
    input_shape = tr_utils.get_input_shape(which_input="audio")
...
def def_all_models(
    num_images, h_params, load_audio_image_model=True, class_weights=None
):
    """
    ...
    input_shape_image = tr_utils.get_input_shape(which_input="image")
    input_shape_lands = tr_utils.get_input_shape(which_input="landmarks_depths")
    input_shape_audio = tr_utils.get_input_shape(which_input="audio")
    input_shape_text_sent = tr_utils.get_input_shape(which_input="text_sentiment")
    ...
    # Freeze the weights of the face and audio models
    for param in face_model_defined.parameters():
        param.requires_grad = False
    for param in audio_model_defined.parameters():
        param.requires_grad = False
    ...
    input_shapes = (
        (input_shape_image, input_shape_lands),
        input_shape_audio,
        input_shape_text_sent,
    )
    print("Input shapes", input_shapes)
    combined_model = (
        CustomModel(
            image_model=(face_model_defined, face_last_layer_num),
            ...
            # sequence_model_layers=sequence_model_layers,
            d_model=h_params.get("d_model", 512),
            nhead=h_params.get("nhead", 8),
            num_encoder_layers=h_params.get("num_encoder_layers", 6),
            num_decoder_layers=h_params.get("num_decoder_layers", 6),
            dim_feedforward=h_params.get("dim_feedforward", 2048),
        )
        .to(device)
        .train()
    )
    ...
    if defined_callbacks is None:
        defined_callbacks = {
            "val": pt_callbacks.Callbacks(
                optimizer=optimiser, model_save_path=MODEL_SAVE_PATH
            ),
...
def train(
    ...
    max_threads=tr_config.MAX_TRAINING_THREADS,
):
    """
    ...
    train_data, test_data = tr_data.split_data(
        test_split_percent=tr_config.test_split_percentage
    )
    (
        all_frames_face_images_X_train,
        all_frames_face_lands_X_train,
        all_extracted_mfcc_list_train,
        all_text_sentiment_X_train,
        all_video_emotions_Y_train,
    ) = train_data
    (
        all_frames_face_images_X_test,
        all_frames_face_lands_X_test,
        all_extracted_mfcc_list_test,
        all_text_sentiment_X_test,
        all_video_emotions_Y_test,
    ) = test_data
    ...
    print(
        f"all_frames_face_images_X_train.shape: {all_frames_face_images_X_train.shape}"
    )
    ...
    all_frames_face_lands_X_train = torch.from_numpy(
        all_frames_face_lands_X_train
    ).float()
    all_extracted_mfcc_list_train = torch.from_numpy(
        all_extracted_mfcc_list_train
    ).float()
    ...
    all_frames_face_images_X_test = torch.from_numpy(
        all_frames_face_images_X_test
    ).float()
    all_frames_face_lands_X_test = torch.from_numpy(
        all_frames_face_lands_X_test
    ).float()
    all_extracted_mfcc_list_test = torch.from_numpy(
        all_extracted_mfcc_list_test
    ).float()
    ...
    num_workers = max_threads if max_threads > 0 else 0
    persistent_workers = (
        num_workers > 0
...
def hyper_parameter_optimise(
    ...
    for path in [best_hp_json_save_path, tuner_csv_save_path, tuner_obj_save_path]:
    ...
    try:
        # Run at least one trial
        best = fmin(
            fn=tuner_utils.train_for_tuning,
            space=search_space_hyperopt,
            algo=tpe.suggest,  # Changed from suggest_grid to tpe.suggest
            max_evals=max(1, tuner_utils.max_trials),
            trials=trials,
            verbose=True,
        ...
        # Cleanup
        if hasattr(trials, "_ids") and hasattr(trials, "attachments"):
            trials._ids = []
            trials.attachments = {}
...
class DataGenerator(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        all_text_sentiment_X,
        all_video_emotions_Y,
        image_augmentation=get_training_augmentation,
    ):
        # Move data to CPU during initialization
        self.X_images = (
            all_frames_face_images_X.cpu()
            if torch.is_tensor(all_frames_face_images_X)
            else all_frames_face_images_X
        )
        self.X_landmark_depth = (
            all_frames_face_lands_X.cpu()
            if torch.is_tensor(all_frames_face_lands_X)
            else all_frames_face_lands_X
        )
        self.X_mfcc = (
            all_extracted_mfcc_list.cpu()
            if torch.is_tensor(all_extracted_mfcc_list)
            else all_extracted_mfcc_list
        )
        self.X_text_sentiment = (
            all_text_sentiment_X.cpu()
            if torch.is_tensor(all_text_sentiment_X)
            else all_text_sentiment_X
        )
        self.Y = (
            all_video_emotions_Y.cpu()
            if torch.is_tensor(all_video_emotions_Y)
            else all_video_emotions_Y
        )
    ...
    def convert_numpy_to_tensor(self, numpy_array):
        ...
        # Don't move to device here - let DataLoader handle it
        img = torch.from_numpy(numpy_array).type(torch.FloatTensor)
        return img
