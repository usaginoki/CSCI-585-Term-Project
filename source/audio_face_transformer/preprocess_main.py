...
import source.audio_face_transformer.utils as combined_utils
import source.audio_face_transformer.tr_config as combined_config
...
def preprocess_images(
    images_list,
    target_frames=round(
        combined_config.FRAME_RATE * combined_config.VIDEO_ANALYSE_WINDOW_SECS
    ),
    print_flag=True,
    show_img=False,
):
    ...
    for image in images_list:
        ...
        results = face_mesh.get_mesh(
            image.copy(),
            return_mesh=show_img,
            upscale_landmarks=False,
        ...
        if results is None:
            ...
            if len(image.shape) > 2:
                ...
                grey_image = image
            grey_image = cv2.resize(
                grey_image, (face_config.FACE_SIZE, face_config.FACE_SIZE)
            )
    ...
    if no_detect_imgs_cnt > (len(all_face_images_X) / 2):
        print(
            f"More than half of the frames in the clip have no face detected. Skipping"
        )
        return None
    ...
    if len(all_face_land_dists_depths_X) < target_frames:
        if print_flag:
            print(
                f"{len(all_face_land_dists_depths_X)} frames detected. Adding {target_frames - len(all_face_land_dists_depths_X)} zero frames for target of {target_frames}"
            )
        for _ in range(target_frames - len(all_face_land_dists_depths_X)):
            all_face_land_dists_depths_X.append(
                np.zeros(face_config.LANDMARK_COMBINATIONS_DEPTHS_CNT)
            )
            all_face_images_X.append(
                np.zeros((face_config.FACE_SIZE, face_config.FACE_SIZE))
            )
...
def preprocess_audio(
    ...
    try:
        ...
        samples, sample_rate = audio_preprocess.clean_single(
            file_or_samples=audio_samples,
            sample_rate=audio_config.READ_SAMPLE_RATE,
            print_flag=print_flag,
        )
        print()

        # Ensure samples are in float64 format
        ...
        if extracted_mfcc.shape[1] < NUM_MFCC:
            input(
                f"Wrong shape of extracted_mfcc{extracted_mfcc.shape}\n This will cause problems. Press enter to continue"
            )

        extracted_mfcc = np.array(extracted_mfcc, dtype=np.float64)
...
def get_sentiment(
    audio, disable_sentiment=combined_config.DISABLE_SENTIMENT, print_flag=True
):
...
def preprocess_single_video(
    ...
    try:
        ...
        # 2. Add debug prints for video conversion
        print(f"Converting video to frames and audio: {video_path}")
        ...
        print(f"Frames extracted: {len(list_of_n_frames[-1])}")
        print(f"Audio clips extracted: {len(list_of_audio_clips[-1])}")

        # 3. Add validation for frames and audio
        if not list_of_n_frames or not list_of_audio_clips:
            print(f"No frames or audio extracted from {video_path}")
            num_threads_running -= 1
            return None
        ...
        result = preprocess_images(
            images_list=list_of_n_frames, print_flag=print_flag, show_img=show_img
        )
        ...
        text_sentiment_X = get_sentiment(audio=list_of_audio_clips, print_flag=True)
        ...
        if label is not None:
            if print_flag:
                print(f"Label: {label}")
            results_thread[video_path.split(os.sep)[-1]] = (
                frames_lands_X,
                frames_faceimages_X,
                extracted_mfcc_X,
                text_sentiment_X,
        ...
        print(f"Error in preprocess_single_video for {video_path}:")
...
def preprocess_videos(
    ...
    for folder in original_videos_folder:
        for file in os.listdir(folder):
            ...
            try:
                ...
                # 1. Add validation for file path
                ...
                # 2. Add validation for file format
                if not file.lower().endswith((".mp4", ".avi", ".mov", ".flv")):
                    print(f"Invalid file format: {file}")
                    num_threads_running -= 1
                    continue

                # 3. Add detailed error handling for emotion label extraction
                try:
                    if predict_mode:
                        emotion_label_softmax = np.array(
                            [0] * len(face_config.EMOTION_INDEX_REVERSE)
                        )
                    else:
                        emotion_parts = file.split("_")
                        if len(emotion_parts) < 3:
                            raise ValueError(
                                f"Invalid filename format - expected format: NAME_ID_EMOTION.mp4, got: {file}"
                            )

                        emotion_key = emotion_parts[2].split(".")[0]
                        if emotion_key not in config.FULL_EMOTION_INDEX_REVERSE:
                            raise ValueError(f"Unknown emotion key: {emotion_key}")

                        emotion_label = config.FULL_EMOTION_INDEX_REVERSE[emotion_key]
                        emotion_label_softmax = face_utils.get_as_softmax(
                            emotion_label, config.NON_SIMPLIFIED_SOFTMAX_LEN
                        )
                        emotion_label_softmax = np.array(emotion_label_softmax)
                except Exception as e:
                    print(f"Error processing emotion label for {file}: {str(e)}")
                    print(
                        f"Available emotion keys: {list(config.FULL_EMOTION_INDEX_REVERSE.keys())}"
                    )
                    num_threads_running -= 1
                    invalid_file_list.append(file)
                    continue

                # 4. Add debug prints before thread creation
                print(f"Starting thread for {file}")
                print(f"Parameters:")
                print(f"- Video path: {video_path}")
                print(f"- N_FFT: {N_FFT}")
                print(f"- NUM_MFCC: {NUM_MFCC}")
                print(f"- HOP_LENGTH: {HOP_LENGTH}")
                print(f"- Emotion label: {emotion_label_softmax}")

                # 5. Modify thread creation to use try-except
                try:
                    _thread.start_new_thread(
                        ...
                        (
                            ...
                            True,  # Enable printing for debugging
                            show_img,
                        ),
                    )
                except Exception as e:
                    print(f"Error creating thread for {file}:")
                    print(f"Exception: {str(e)}")
                    traceback.print_exc()
                    num_threads_running -= 1
                    invalid_file_list.append(file)
            ...
            while num_threads_running >= MAX_THREADS:
                if print_flag:
                    if not printed_flag:
                        print(
                            f"..........Waiting for threads to end. Num threads: {num_threads_running}"
                        )
                        printed_flag = True
                    if last_so_far_cnt != so_far_cnt:
                        last_so_far_cnt = so_far_cnt
                        print(f"{detected_cnt}/{so_far_cnt} videos detected")
                        stats_string = audio_utils.get_progress_stats(
                            start_memory, start_time, so_far_cnt, all_cnt
                        )
                        print(f"\n" + stats_string)
        ...
        for i, result_key in enumerate(results_thread):
            print(
                f"{folder.split(os.sep)[-3:-1]} Result: {i}, total: {total_frames_detected}: {result_key}"
            )
    ...
    print(
        "all_train_test_classification_Y shape: "
        + str(len(all_train_test_classification_Y))
    )

    if not predict_mode:
        print("\nSaving data to file")
        np.save(
            os.path.join(output_path, "all_frames_face_lands_X.npy"),
            all_frames_face_lands_X,
        )
        np.save(
            os.path.join(output_path, "all_frames_face_images_X.npy"),
            all_frames_face_images_X,
        )
        np.save(
            os.path.join(output_path, "all_extracted_mfcc_list.npy"),
            all_extracted_mfcc_list,
        )
        np.save(
            os.path.join(output_path, "all_text_sentiment_X.npy"), all_text_sentiment_X
        )
        np.save(
            os.path.join(output_path, "all_video_emotions_Y.npy"), all_video_emotions_Y
        )
        np.save(
            os.path.join(output_path, "all_train_test_classification_Y.npy"),
            all_train_test_classification_Y,
        )
...
def load_preprocessed_data():
    all_frames_face_lands_X = np.load(
        os.path.join(config.PREPROCESSED_AV_FOLDER_PATH, "all_frames_face_lands_X.npy")
    )
    all_frames_face_images_X = np.load(
        os.path.join(config.PREPROCESSED_AV_FOLDER_PATH, "all_frames_face_images_X.npy")
    )
    all_extracted_mfcc_list = np.load(
        os.path.join(config.PREPROCESSED_AV_FOLDER_PATH, "all_extracted_mfcc_list.npy")
    )
    all_text_sentiment_X = np.load(
        os.path.join(config.PREPROCESSED_AV_FOLDER_PATH, "all_text_sentiment_X.npy")
    )
    all_video_emotions_Y = np.load(
        os.path.join(config.PREPROCESSED_AV_FOLDER_PATH, "all_video_emotions_Y.npy")
    )
    all_train_test_classification_Y = np.load(
        os.path.join(
            config.PREPROCESSED_AV_FOLDER_PATH, "all_train_test_classification_Y.npy"
        )
    )
    ...
    all_train_test_classification_Y = audio_utils.shuffle_train_data(
        all_train_test_classification_Y
    )
...
def split_data(test_split_percent=audio_config.test_split_percentage):
    def classify(data, classification):
        ...
        print(
            "split data: "
            + str(classified_data_train.shape)
            + " "
            + str(classified_data_test.shape)
        )
    ...
    print("\nSplitting data")
    all_frames_face_images_X_train, all_frames_face_images_X_test = classify(
        all_frames_face_images_X, all_train_test_classification_Y
    )
    all_frames_face_lands_X_train, all_frames_face_lands_X_test = classify(
        all_frames_face_lands_X, all_train_test_classification_Y
    )
    all_extracted_mfcc_list_train, all_extracted_mfcc_list_test = classify(
        all_extracted_mfcc_list, all_train_test_classification_Y
    )
    all_text_sentiment_X_train, all_text_sentiment_X_test = classify(
        all_text_sentiment_X, all_train_test_classification_Y
    )
    all_video_emotions_Y_train, all_video_emotions_Y_test = classify(
        all_video_emotions_Y, all_train_test_classification_Y
    )
    ...
    all_frames_face_images_X_train = np.repeat(
        all_frames_face_images_X_train[:, :, np.newaxis, :, :], 3, axis=2
    )
    ...
    all_frames_face_images_X_test = np.repeat(
        all_frames_face_images_X_test[:, :, np.newaxis, :, :], 3, axis=2
    )
