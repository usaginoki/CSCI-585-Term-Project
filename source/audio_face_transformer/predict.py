...
import source.audio_face_transformer.preprocess_main as tr_preprocess_main
import source.audio_face_transformer.model as model
...
def apply_bounding_box(image, prediction_softmax, tl_xy, br_xy):  # ! the same
    ...
    face_input = cv2.rectangle(
        face_input, tl_xy, br_xy, (0, 255, 0), max(face_input.shape[0] // 500, 1)
    )
    cv2.putText(
        img=face_input,
        text=pred_string,
    ...
    wait_time = round(round(1000 / tr_config.FRAME_RATE))
    cv2.waitKey(wait_time)


def predict_video(
    video_path,
    video_buffer_folder=config.MAIN_PATH + "VideoBufferFolder" + os.sep,
    ...
    video_buffer_folder = (
        video_buffer_folder + video_path.split(os.sep)[-1].split(".")[0] + os.sep
    )
    video_name = video_path.split(os.sep)[-1]

    # Convert to videos of x secs
    durations, filename_list = tr_utils.split_video_into_equal_parts(
        video_path, video_buffer_folder
    )

    # Load tuned hyperparameters for audio and combined models to preprocess data
    # Audio
    audio_best_hyperparameters = face_utils.load_dict_from_json(
    ...
    best_hyperparameters = face_utils.load_dict_from_json(
        config.AUDIO_FACE_TRANSFORMER_BEST_HP_JSON_SAVE_PATH
    )
    print(f"Combined best hyperparameters, {best_hyperparameters}")
    result, valid_file_list, invalid_file_list = tr_preprocess_main.preprocess_videos(
        N_FFT=N_FFT,
        NUM_MFCC=NUM_MFCC,
    ...
    if show_image == 1:
        annotated_frames = tr_preprocess_main.all_annotations_X
        tl_br_xy_list = tr_preprocess_main.tl_br_xy_list
    ...
    frames_face_images_X = (
        torch.from_numpy(frames_face_images_X).float().to(config.device)
    )
    frames_face_lands_X = (
        torch.from_numpy(frames_face_lands_X).float().to(config.device)
    )
    extracted_mfcc_list = (
        torch.from_numpy(extracted_mfcc_list).float().to(config.device)
    )
    text_sentiment_X = torch.from_numpy(text_sentiment_X).float().to(config.device)

    # Predict
    predictions = combined_model(
        frames_face_images_X, frames_face_lands_X, extracted_mfcc_list, text_sentiment_X
    )
    ...
    for i in range(len(predictions)):
        ...
        dur_i = start
        durations[dur_i][0] = (
            f"{int(durations[dur_i][0] / 60)}:{int(durations[dur_i][0] % 60)}"
        )
        durations[dur_i][1] = (
            f"{int(durations[dur_i][1] / 60)}:{int(durations[dur_i][1] % 60)}"
        )

        for j in range(len(predictions[i])):
            durations[dur_i].append(
                titles[j] + ": " + str(round(predictions[i][j] * 100, 1)) + "%"
            )
        ...
        print(
            ...
            audio_utils.get_softmax_probs_string(
                predictions[i], list(config.EMOTION_INDEX.values())
            ),
        )
    ...
    with open(
        config.OUTPUT_FOLDER_PATH + f"video_output_{video_name.split('.')[0]}.csv", "w"
    ) as csv_file:
    ...
    for i in range(len(sum_probs)):
        print(
            f"{config.EMOTION_INDEX[i]} : {round((sum_probs[i] / len(predictions)) * 100, 2)}%"
        )
    ...
    # audio_utils.convert_video_to_audio(
    #     video_path, config.OUTPUT_FOLDER_PATH + "audio.wav"
    # )
    ...
    # with open(
    #     config.OUTPUT_FOLDER_PATH + "transcript_word_occurrences.csv", "w"
    # ) as csv_file:
    #     writer = csv.writer(csv_file, lineterminator="\n")
    ...
    # )
    # print(
    #     "Check output folder for the output files, 'video_transcript.txt', 'transcript_word_occurrences.csv' and 'video_output.csv'"
    # )
