...
def load_saved_objects(training_stats_path):
    ...
    if os.path.exists(training_stats_path) is False:
        raise ValueError(
            f"training_stats_path {training_stats_path} does not exist, cannot load the saved objects. Consider fine tuning instead, if you have a saved model"
        )
...
class Callbacks:
    def __init__(
        ...
        if continue_training:
            assert (
                training_stats_path is not None
            ), "training_stats_path must be provided if continue_training is True"
    ...
    def model_checkpoint(
        ...
        if self._model_save_path is None:
            if model_save_path is None:
                raise ValueError(
                    "model_save_path must be provided in the constructor or in the function call"
                )
        ...
        if (mode == "max" and self._checkpoint_last_best < monitor_value) or (
            ...
            if verbose:
                print(
                    f"{indicator_text} monitor value improved from {self._checkpoint_last_best} to {monitor_value}"
                )
            ...
            if verbose:
                print(
                    f"{indicator_text} monitor value did not improve from {self._checkpoint_last_best}"
                )
    ...
    def reduce_lr_on_plateau(
        ...
        if improved_flag:
            ...
            if verbose:
                print(
                    f"{indicator_text} monitor value improved from {self._reduce_lr_last_best} to {monitor_value}"
                )
            ...
            if self._reduce_lr_last_lr * factor > min_lr:
                if self._reduce_lr_num_bad_epochs >= patience:
                    ...
                    if verbose:
                        print(
                            f"{indicator_text} lr reduced to {self._reduce_lr_last_lr}. monitor value did not improve for {patience} epochs from {self._reduce_lr_last_best}"
                        )
                else:
                    if verbose:
                        print(
                            f"{indicator_text} monitor value did not improve for {self._reduce_lr_num_bad_epochs} epochs from {self._reduce_lr_last_best}. Waiting for {patience - self._reduce_lr_num_bad_epochs} epochs more."
                        )
    ...
    def early_stopping(
        ...
        if (mode == "max" and self._early_stopping_last_best < monitor_value) or (
            ...
            if verbose:
                print(
                    f"{indicator_text} monitor value improved from {self._early_stopping_last_best} to {monitor_value}"
                )
            ...
            if self._early_stopping_num_bad_epochs >= patience:
                if verbose:
                    print(
                        f"{indicator_text} monitor value did not improve for {patience} epochs. Stopping training."
                    )
                return True
            else:
                if verbose:
                    print(
                        f"{indicator_text} monitor value did not improve for {self._early_stopping_num_bad_epochs} epochs from {self._early_stopping_last_best}. Waiting for {patience - self._early_stopping_num_bad_epochs} epochs more."
                    )
                return False
