...
def evaluate(
    model: torch.nn.Module, val_loader: torch.utils.data.DataLoader, device=device
):
    model.eval()
    outputs = []
    with torch.no_grad():
        ...
        while num_retry < max_retry:
            try:
                ...
                for batch in progress_bar:
                    ...
                    if not flag:
                        raise RuntimeError(
                            f"Training step failed {num_retries_inner} times"
                        )
...
class CustomModelBase(torch.nn.Module):
    ...
    def training_step(self, batch: list):
        ...
        loss = self.loss_function(
            out, labels, weight=self.class_weights
