...
class HyperTunerUtils:
    ...
    def __init__(
        ...
        if load_if_exists:
            ...
            if os.path.exists(tuner_obj_save_path):
                os.remove(
                    tuner_obj_save_path
                )  # easy way to clear the trials if we want to start from scratch
    ...
    def save_trial_to_csv(self, trial_details: dict, tuner_csv_save_path=None):
        """
    ...
    def get_best_trial_details_from_csv(self):
        ...
        if os.path.exists(self.tuner_csv_save_path):
            with open(self.tuner_csv_save_path, "r") as csvfile:
                reader = csv.DictReader(csvfile)
                best_trial_details = max(
                    reader, key=lambda row: float(row[self.tune_target])
                )
    ...
    def set_hp_variable(self, hp_name: str, hp_ranges=None):
        """
        ...
        if choice_type == "choice":
            ...
            if len(hp_range) > 1:
                if type(hp_range[0]) == int:
                    hp_val = np.round(
                        np.linspace(hp_range[0], hp_range[1], num=hp_range[2]).astype(
                            int
                        )
                    )
    ...
    def suggest_grid(self, new_ids, domain, trials, seed):
        ...
        for i, new_id in enumerate(new_ids):
            ...
            new_trail = trials.new_trial_docs(
