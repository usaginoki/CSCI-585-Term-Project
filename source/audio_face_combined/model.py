...
class CustomModel(CustomModelBase):
    ...
    def __init__(
        ...
        dense_input_size_post_comb = (
            sequence_model_dense_units + audio_last_layer_num + input_shapes[2]
        )
        ...
        if sequence_model:
            self.lstm_layers = nn.LSTM(
                input_size=face_last_layer_num,
                hidden_size=sequence_model_dense_units,
                num_layers=sequence_model_layers,
        ...
        self.fc = nn.Sequential(
            nn.Linear(
                dense_input_size_post_comb, dense_units
            ),  # Update the input size for the first dense layer
    ...
    if defined_callbacks is None:
        defined_callbacks = {
            "val": pt_callbacks.Callbacks(
                model_save_path=MODEL_SAVE_PATH, optimizer=optimiser
            ),
...
def train(
    ...
    continue_training=True,
    max_threads=combined_config.MAX_TRAINING_THREADS,
):
    """
    ...
    train_data, test_data = combined_data.split_data(
        test_split_percent=combined_config.test_split_percentage
    )
    (
